...
![image](https://i.imgur.com/ubyt3ye.png)
## Express
...
![image](https://i.imgur.com/mS7GYAV.png)
...
- However, the installed version of express can also be one that has a larger patch number (the last number), or a larger minor number (the middle number). 
...
2. The second route defines an event handler, that handles HTTP GET requests made to the notes path of the application:
    ...
    ![image](https://i.imgur.com/a6ety5o.png)
...
- This can happen with Postman if the type of body is not defined correctly, if the Content-Type header is set to text/plain, the server appears to only receive an empty object.
